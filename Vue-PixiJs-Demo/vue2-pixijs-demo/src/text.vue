<template>
    ...
    methods:{
      ...
      createStickerCanvas(){
        ...
        this.pixiapp = new PIXI.Application({
          width : window.innerWidth,//指定引擎画布宽度
          height : window.innerHeight,//指定引擎画布高度
          backgroundColor : 0xc3a072, //指定引擎画布背景颜色
          resolution : window.devicePixelRatio || 1 //指定像素分辨率 window.devicePixelRatio获取当前设备的像素比例，如果没有就为1:1
        })
        //将PIXI渲染器放置到页面的指定元素中
      ...
      drawBackgroup(){
        //加载背景图片
        const raindropBackgroupImage = require('@/assets/rainydaybackgroup.png')
        //创建背景纹理纹理
   ...
   * {
    ...
    #login-form {
      ...
      text-align: center;
      z-index: 1;
      background-image: url('@/assets/borderbackgroup.png');
      background-size: contain; /* 使背景图片铺满整个 div */
